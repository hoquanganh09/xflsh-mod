...
abstract class BaseServer extends Model
{
     ...
     * @return array
     */
    public static function baseFaultNodeNames(string $checkKey): array
    {
        $result = [];
     ...
     */
    public
    static function baseNodes(string $type, string $userKey, string $checkKey, string $pushKey): Collection
    {
        ...
        foreach ($servers as $server) {
            ...
            foreach ($onlineData as $ip => $onlineJSON) {
                ...
                if (isset($onlineItem->time) && time() - $onlineItem->time <= 300) {
            ...
            foreach ($ips as $ip) {
                ...
                if ((time() - 300) >= $ipLastCheckAt) {
                    $ipAvailableStatus = 0;
                } else if ((time() - 300) >= $ipLastPushAt) {
                    $ipAvailableStatus = 1;
                } else {
                    $ipAvailableStatus = 2;
                }
     ...
     */
    public
    function findAvailableUsers(): Collection
    {
